# %%
# Report mld-mini-2, Scenario B : joint (c_k, c_eps) tuning on the 60-level
# GlobalFlexibleMLDLearningSetup, loss = squared MLD error (see
# scripts/Reports/report-mld-mini-1/section3b_ck_ceps_mld_scenario.py for the 15-level
# version this mirrors -- same structure, heavier grid).
#   - 3 gradient-descent runs from different random starts -> 2D loss landscape + 3
#     trajectories, converging (or not) towards the true (c_k, c_eps)
#   - MLD snapshot: target / initial guess / optimized, for run 0
#   Budget: pred_iter kept small (5) and n_steps capped at 200 -- fast/cheap
#   regression-style check, not a long sweep like report-2.
...
from common import spin_up_global4deg_mld_mini2, make_diff_step, set_vars, rollout, mld_agg_function, TRUE_PARAMS

g4d, step_jit = spin_up_global4deg_mld_mini2(200)
...
mld0 = target_state.variables.mld
print(f"target mld NaN frac: {float(jnp.isnan(mld0).mean()):.4f}  "
      f"finite range: [{float(jnp.nanmin(mld0)):.2f}, {float(jnp.nanmax(mld0)):.2f}]")
...
# sanity probe: loss/gradient scale at true params, before committing to the sweep/GD
# hyperparameters below (mld-mini-1 found MLD-loss gradients ~1e4-1e5, much bigger than
# report-1's temp-loss scale -- the 60-level grid is a different/heavier model, so
# checked here rather than assumed).
probe_loss, probe_grad = loss_grad_fn(jnp.array([c_k_true, c_eps_true]))
print(f"probe @ true params: loss={float(probe_loss):.4e}  grad={np.asarray(probe_grad)}")

# %%
# 2D scan (kept modest -- 15x15, matching mld-mini-1)
n_grid = 15
c_k_grid = jnp.linspace(0.02, 0.18, n_grid)
c_eps_grid = jnp.linspace(0.3, 1.1, n_grid)

losses_grid = np.zeros((n_grid, n_grid))
for i, ck in enumerate(tqdm(c_k_grid, desc="scanning c_k x c_eps (mld-mini-2)")):
    for j, ce in enumerate(c_eps_grid):
        losses_grid[i, j] = loss_grid_fn(jnp.array([ck, ce]))

# %%
# 3 gradient-descent runs from different random starts. optax.adam, same reasoning as
# mld-mini-1: raw MLD-loss gradients are large/badly-scaled for a fixed clipped step,
# adam adapts per-parameter from each parameter's own gradient-magnitude history.
adam_lr = 0.01
...
for seed in range(3):
    ...
    traj = [params]
    for _ in tqdm(range(n_steps), desc=f"GD run {seed} (mld-mini-2)"):
...
# %%
out_dir = f"{PRP}Results/Report/figures/report-mld-mini-2"
...
ax.set_title(f"(c_k, c_eps) MLD-loss landscape, 3 GD runs ({pred_iter}-step rollout, global_4deg_mld nz=60)")
ax.set_xlabel("c_k")
ax.set_ylabel("c_eps")
ax.legend()
fig.tight_layout()

out_path = f"{out_dir}/section3b_ck_ceps_mld_landscape.png"
...
for ax in axs:
    ax.set_xlabel("xt")
axs[0].set_ylabel("yt")
fig.suptitle(f"MLD bias after {pred_iter} steps (c_k/c_eps scenario, global_4deg_mld nz=60)")

out_path = f"{out_dir}/section3b_ck_ceps_mld_snapshot.png"
fig.savefig(out_path, dpi=150)
print(f"Saved figure to {out_path}")
