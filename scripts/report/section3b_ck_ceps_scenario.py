# %%
# Report Section 3, Scenario B : joint (c_k, c_eps) tuning on global_4deg.
#   - 3 gradient-descent runs from different random starts -> 2D loss landscape + 3
#     trajectories, all converging to the true (c_k, c_eps)
...
from scripts.report.common import spin_up_global4deg, make_diff_step, set_vars, rollout, TRUE_PARAMS
...
ax.set_title(f"(c_k, c_eps) loss landscape, 3 GD runs ({pred_iter}-step rollout, global_4deg)")
ax.set_xlabel("c_k")
ax.set_ylabel("c_eps")
ax.legend()
fig.tight_layout()

out_path = f"{PRP}Results/Report/figures/section3b_ck_ceps_landscape.png"
...
out_path = f"{PRP}Results/Report/figures/section3b_ck_ceps_temp_snapshot.png"
fig.savefig(out_path, dpi=150)
print(f"Saved figure to {out_path}")
