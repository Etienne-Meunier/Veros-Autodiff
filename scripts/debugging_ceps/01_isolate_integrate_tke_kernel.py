# %%
# Regression check: gradient of integrate_tke_kernel's own outputs (tke, tke_surf_corr,
# tke_diss) w.r.t. c_eps, autodiff vs central finite difference, in isolation (no other
# model physics). Should match closely under both tke_mxl_choice values.
...
from veros.core.tke import integrate_tke_kernel
...
# %%
# For a given tke_mxl_choice : call integrate_tke_kernel once on the spun-up state, at
# c_eps = 0.6 (away from the true value 0.7), and compare autodiff vs central finite
# difference for each of the kernel's output fields independently.
def check(tke_mxl_choice, test_val=0.6, eps=1e-4):
    acc = build_spun_up_acc(tke_mxl_choice)

    def kernel_call(value, state):
        n_state = set_c_eps(state, value)
        return integrate_tke_kernel(n_state)

    out_plus = kernel_call(jnp.array(test_val) + eps, acc.state)
    out_minus = kernel_call(jnp.array(test_val) - eps, acc.state)

    print(f"\n=== tke_mxl_choice={tke_mxl_choice}  (c_eps={test_val}, eps={eps:.0e}) ===")
    for field in out_plus._fields:
        def loss(value, field=field):
            out = kernel_call(value, acc.state)
