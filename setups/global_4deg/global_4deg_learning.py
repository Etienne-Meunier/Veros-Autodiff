...
"""
Differentiable variant of the built-in `global_4deg` Veros setup, adapted the same
way `setups/acc/acc_learning.py` was adapted from `setups/acc/acc.py`:
 ...
 - `enable_streamfunction = False` : avoids the untested/unpatched barotropic
   ...
   Keeps the model on the `solve_pressure.py` path, which is the one patched
   with `lax.cond` for differentiability.
 - `eq_of_state_type = 3` (nonlin2) instead of `5` (gsw/TEOS-10) : `gsw.py`'s
   `gsw_rho`/`gsw_drhodT`/`gsw_drhodS`/`gsw_drhodP` call `sqrt(salt)` with no
   floor, which reproduces the NaN-gradient bug `safe_sqrt` was built to fix
   in eke/tke, at every masked/land cell (salt == 0). `nonlinear_eq2.py` has
   no sqrt calls at all and is the equation of state already exercised by the
   ACC learning notebooks. Revisit if this experiment specifically needs
   TEOS-10 (would require patching `gsw.py`'s unguarded sqrt calls first).
 - All diagnostics disabled (`diagnostics.clear()`) : diagnostics do file I/O
   and build `xarray.Dataset` objects from Python-side state, which doesn't
   survive `jit` tracing.
"""
