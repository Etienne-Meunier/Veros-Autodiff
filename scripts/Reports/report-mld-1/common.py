"""Shared helpers for the mld gradient-check / tuning scripts.
...
the setup class (GlobalFlexibleMLDLearningSetup instead of GlobalFourDegreeSetup) and
the aggregation target (mld instead of temp, see mld_agg_function below).
"""
...
def spin_up_global4deg_mld(warmup_steps=200):
    ...
    for _ in tqdm(range(warmup_steps), desc="spin-up (mld)"):
...
def mld_agg_function(state, target_state):
    ...
    (mld_from_prho returns NaN at land/degenerate columns -- see
    ...
    (same pattern as _safe_mean_where in the setup file).
    """
