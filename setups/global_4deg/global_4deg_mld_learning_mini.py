...
"""
`global_4deg_learning.py`'s `GlobalFourDegreeSetup`, unchanged, plus the MLD
mixed-layer-depth diagnostic from `global_4deg_mld_learning.py` bolted on top.

Deliberately NOT based on `global_4deg_mld_learning.py`'s `GlobalFlexibleMLDLearningSetup`
(ETOPO5-interpolated topography, nz=60, `assets_mld.json`) -- that setup is a different,
heavier grid. This one keeps every setting/grid/forcing routine from
`global_4deg_learning.py` (nx=90, ny=40, nz=15, `assets.json`-based bathymetry) exactly
as-is; the only addition is the `mld` diagnostic, reusing `get_index_mld`/
`mld_from_index` from `global_4deg_mld_learning.py` rather than reimplementing them (see
that module's docstring for why the index-selection/value-gather split is NaN-gradient
safe).
"""
...
from setups.global_4deg.global_4deg_mld_learning import get_index_mld, mld_from_index, update_mld_moving_average

BASE_PATH = os.path.dirname(os.path.realpath(__file__))
DATA_FILES = veros.tools.get_assets("global_4deg", os.path.join(BASE_PATH, "assets.json"))


class GlobalFourDegreeMLDMiniSetup(VerosSetup):
    """`GlobalFourDegreeSetup` (see global_4deg_learning.py) + an `mld` diagnostic."""

    # depth (m, negative down) above which the MLD reference density is taken; matches
    # global_4deg_mld_learning.py. Recomputed from zt on every after_timestep call
    # (see get_index_mld) -- not cached.
    ...
    def set_parameter(self, state):
        ...
        settings.identifier = "global_4deg_mld_learning_mini"
        settings.description = "global_4deg_learning.py's setup, unchanged, + mld diagnostic"
        ...
        # c_k, c_eps are state Variables (not settings, see veros/variables.py) -- left
        # at their built-in defaults (0.1, 0.7) here, same as global_4deg_learning.py.
    ...
    def after_timestep(self, state):
        ...
        ridx, i_below, i_above, well_defined = get_index_mld(vs.prho, vs.maskT, vs.zt, self.mld_reference_depth)
        mld = mld_from_index(vs.prho, vs.zt, ridx, i_below, i_above, well_defined)
