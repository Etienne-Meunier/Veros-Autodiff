"""Shared helpers for the mld-mini gradient-check / tuning scripts.

Same structure as scripts/Reports/report-1/common.py (temp-loss version): spin up a
warmed-up state, wrap the step for differentiable rollout via jax.checkpoint (combine
with jax.lax.scan at the call site for long rollouts -- see
scripts/gradient_routines/README.md), set_vars/rollout helpers. Only difference here:
the setup class (GlobalFourDegreeMLDMiniSetup -- global_4deg_learning.py's config
unchanged, + the mld diagnostic, see setups/global_4deg/global_4deg_mld_learning_mini.py)
and the aggregation target (mld instead of temp, see mld_agg_function below, same
pattern as scripts/Reports/report-mld-1/common.py).
"""
...
def spin_up_global4deg_mld_mini(warmup_steps=200):
    g4d = GlobalFourDegreeMLDMiniSetup()
    g4d.setup()
    ...
    for _ in tqdm(range(warmup_steps), desc="spin-up (mld-mini)"):
